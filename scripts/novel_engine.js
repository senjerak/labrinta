...
Character.prototype.say = function (str) {
  ...
  if (this.name != "") {
    htmlStr +=
      '<span style="color: ' + this.color + '">' + this.name + ":</span><br />";
  }
  if (str.indexOf("{{") >= 0) {
...
function novel_textEntity_display(obj, param) {
  ...
  // set the text
  el.innerHTML = obj.text.replace(/{{(.*?)}}/g, novel_interpolator);
...
Position.prototype.equals = function (other) {
  return (
    ...
    this.xAnchor == other.xAnchor &&
    this.yAnchor == other.yAnchor
  );
};
...
function novel_setAlpha(domRef, alpha) {
  if (alpha != 1.0) {
    domRef.style.filter = "alpha(opacity=" + Math.floor(alpha * 100) + ")";
...
function show(param) {
  if (param.constructor == Character || param.constructor == TextBlock) {
    param.display(null);
    param.show(true);
  }
}
...
function novel_findMatchingElse() {
  ...
  while (
    !((item == elsePart || item == endIf) && currLevel == novel.ifLevel) &&
...
function novel_findMatchingEndIf() {
  ...
  var item = novel_script[f];
  while (
    !(item == endIf && currLevel == novel.ifLevel) &&
    f < novel_script.length
  ) {
    if (item == ifStatement) {
      currLevel++;
